...
function generateHtmlContent(a) {
  for (var b = '<ul class="panel-list">', d = 0; d < a.Items.length; d++) {
    ...
    'link' == c.type
      ...
      : 'button' == c.type &&
        ((b += '<span class="panel-list-item-icon ' + c.icon + '" ></span>'),
        (b += '<button onclick="' + c.onclick1 + '">' + c.name1 + '</button>' + '<button onclick="' + c.onclick2 + '"style="float: right;"' + '">' + c.name2 + '</button>'))
  }
  for (var b2 = '<ul class="panel-list">', d = 0; d < a.Items2.length; d++) {
    var c = a.Items2[d],
    b2 = b2 + '<li class="panel-list-item"><div>'
    'link' == c.type
      ? ((b2 += '<span class="panel-list-item-icon ' + c.icon + '" ></span>'),
        (b2 += '<a href="' + c.href + '">' + c.name + '</a>'))
      : 'button' == c.type &&
        ((b2 += '<span class="panel-list-item-icon ' + c.icon + '" ></span>'),
        (b2 += '<button onclick="' + c.onclick + '">' + c.name + '</button>'))
  }

  return b + '</ul>' + b2 + '</ul>' + '<div id="infotable"> <template> <v-table is-horizontal-resize style="width:100%" :columns="columns" :table-data="tableData" :show-vertical-border="false"></v-table> </template> </div>'
}
